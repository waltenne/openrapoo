...
impl BatteryProvider for SysfsProvider {
    ...
    fn query(&self, device: &DeviceIdentity, log: &mut Vec<String>) -> Option<BatteryReading> {
        ...
        for entry in entries.flatten() {
            ...
            if supply_type != "Mouse"
                && supply_type != "Keyboard"
                && supply_type != "Battery"
                && !entry_name.contains("hid")
            {
                continue;
            }
            ...
            log.push(format!(
                "Entrada sysfs validada para '{}': {entry_name}",
                device.name
            ));
...
fn is_device_match(
    ...
    if dev_lower.contains("e9050") {
        return combined.contains("e9050")
            || (combined.contains("keyboard") && !combined.contains("mouse"));
    }

    if dev_lower.contains("mt760") {
        return combined.contains("mt760")
